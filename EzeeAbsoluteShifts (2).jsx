...
const cashDropFor = (shift, code) => code===BASE_CCY ? moneyVal(shift?.cashDrop) : 0;
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const buildCloseBalances = (shift=activeShift, drawer=selected) => {
    const shiftTxns = (txns||[]).filter(t => t.shiftId===shift?.id);
    const openingCcys = Object.keys(shift?.openingCcyBals||{});
    const fxCodes = [...new Set(shiftTxns.filter(t=>t.fxCcy).map(t=>t.fxCcy))];
    const drawerCcys = [...new Set([BASE_CCY, ...openingCcys, ...fxCodes])];
    const openingFor = code => {
      if (shift?.openingCcyBals && shift.openingCcyBals[code] !== undefined) return moneyVal(shift.openingCcyBals[code]);
      if (code===BASE_CCY) return shift ? moneyVal(shift.openingBal ?? drawer?.balance ?? 0) : 0;
      return moneyVal(drawer?.ccyBalances?.[code] ?? 0);
    };
    const sysByCode = Object.fromEntries(drawerCcys.map(code => {
      const txnTotal = shiftTxns.reduce((sum,t) => {
        if (code===BASE_CCY) return sum + (!t.fxCcy ? (t.amount||0) : 0);
        return sum + (t.fxCcy===code ? (t.fxAmt||0) : 0);
      }, 0);
      return [code, openingFor(code) + txnTotal];
    }));
    return { shiftTxns, drawerCcys, sysByCode };
  };
  const { sysByCode: activeSysByCode } = buildCloseBalances();
  const sysBal      = activeSysByCode[BASE_CCY] || 0;
  const usdCounted  = moneyVal(ccyCounts[BASE_CCY]);
  ...
  const handleClose = () => {
    ...
    const { shiftTxns, drawerCcys, sysByCode } = buildCloseBalances();
    const usdRows    = shiftTxns.filter(t=>!t.fxCcy);
    const fxCodes    = drawerCcys.filter(code=>code!==BASE_CCY);
    const countedByCode = Object.fromEntries(drawerCcys.map(code => [
      code,
      ccyCounts[code] !== undefined ? moneyVal(ccyCounts[code]) : sysByCode[code] || 0
    ]));
    const closeDrop = Math.max(0, Math.min(moneyVal(drop), countedByCode[BASE_CCY] || 0));
    const endingByCode = Object.fromEntries(drawerCcys.map(code => [
      code,
      countedByCode[code] - (code===BASE_CCY ? closeDrop : 0)
    ]));
    const closingBal = endingByCode[BASE_CCY] || 0;
    const ccySummary = [
      { code:BASE_CCY,
      ...
      ...fxCodes.map(code => {
        ...
        return { code, inCount:inR.length, outCount:outR.length, totIn, totOut, bal:countedByCode[code] || 0 };
      })
    ];
    setShifts(p => p.map(s => s.id===selected.currentShift
      ...
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt, balance:closingBal, ccyBalances:endingByCode }
      : d));
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${cashDropFor(s,cs.code).toFixed(2)}`, bold:false },
                    { label:"Ending Balance",                 count:"",          amount:`${cs.code} ${(cs.bal-cashDropFor(s,cs.code)).toFixed(2)}`, bold:true },
                  ].map((row,i) => (
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.bal-cashDropFor(s,cs.code)).toFixed(2)}`).join("\n")
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom:`1px solid ${T.rowBorder}`, verticalAlign:"top" }}
                      ...
                      <td style={{ padding:"10px 10px", textAlign:"right", color:T.txtMid }}>
                        {mlPrint(s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${cashDropFor(s,cs.code).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`)}
                      </td>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.bal-cashDropFor(s,cs.code)).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} 0.00`).join("\n") : "USD 0.00";
                      const dropB   = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${cashDropFor(s,cs.code).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`;
                      return (
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            const { drawerCcys, sysByCode } = buildCloseBalances();
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {drawerCcys.map(code => {
                  const drawerBal = ccyCounts[code] !== undefined ? moneyVal(ccyCounts[code]) : sysByCode[code] || 0;
                  const cashDrop  = code===BASE_CCY ? moneyVal(drop) : 0;
        ...
        {step===2 && (<>
          ...
          {(() => {
            ...
            return drawerCcys.map((code, ci) => {
              ...
              const drawerBal = ccyCounts[code] !== undefined ? moneyVal(ccyCounts[code]) : sysByCode[code] || 0;
              const cashDrop  = isUSD ? Math.max(0, moneyVal(drop)) : 0;
              const endingBal = drawerBal - cashDrop;
              const rows2 = [

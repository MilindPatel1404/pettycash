...
function CreateModifyPage({ drawers, setDrawers }) {
  ...
  const openEdit = (d) => {
    if (drawerInUse(d)) {
      window.alert("Close the active shift before editing this drawer.");
      return;
    }
  ...
  const toggleStatus = id => setDrawers(p => p.map(d => {
    if (d.id !== id) return d;
    if (drawerInUse(d)) {
      window.alert("Close the active shift before changing this drawer status.");
      return d;
    }
    return {...d, status:d.status==="Active"?"Inactive":"Active"};
  }));
  const deleteDrawer = target => {
    if (drawerInUse(target)) {
      window.alert("Close the active shift before deleting this drawer.");
      return;
    }
    setDrawers(p => p.filter(x => x.id!==target.id));
  };
              ...
              <tr key={d.id} style={{ borderBottom:`1px solid ${T.rowBorder}` }}
                ...
                <td style={{ padding:"11px 14px" }}>
                  <div style={{ display:"flex", gap:8 }}>
                    ...
                    <button onClick={() => deleteDrawer(d)} style={{ background:"none", border:"none", cursor:"pointer", color:T.txtLight, padding:2, display:"flex" }}>{IC.trash}</button>
                  </div>
                </td>
              </tr>
            ))}
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const mismatch    = closeAccounting.hasUsdCount && variance !== 0;
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${(cs.opening||0).toFixed(2)}`, bold:true  },
                    ...
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${(cs.cashDrop||0).toFixed(2)}`, bold:false },
                    { label:"Ending Balance",                 count:"",          amount:`${cs.code} ${(cs.endingBal ?? cs.bal).toFixed(2)}`, bold:true },
                  ].map((row,i) => (
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.endingBal ?? cs.bal).toFixed(2)}`).join("\n")
                    : `USD ${(s.closingBal||0).toFixed(2)}`;
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.opening||0).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                  const cashDrop = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.cashDrop||0).toFixed(2)}`).join("\n")
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.endingBal ?? cs.bal).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.opening||0).toFixed(2)}`).join("\n") : "USD 0.00";
                      const dropB   = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.cashDrop||0).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`;
                      return (
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            return closeAccounting.summary.map(({ code, expected }) => (
              <div key={code} style={{ marginBottom:14 }}>
                <R2>
          ...
          {(() => {
            return (
              <>
                ...
                {closeAccounting.summary.map(({ code, counted, cashDrop, endingBal }) => {
                  return (
                    <div key={code} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:12, alignItems:"center" }}>
                      <div style={{ padding:"8px 12px", border:`1px solid ${T.inputBdr}`, borderRadius:4, fontSize:13, color:T.txtMid, background:"#FAFAFA" }}>{code} {counted.toFixed(2)}</div>
                      {code==="USD"
        ...
        {step===2 && (<>
          ...
          {(() => {
            return closeAccounting.summary.map((cs, ci) => {
              const rows2 = [
                { label:"Starting Balance",              count:"",          amount:`${cs.code} ${cs.opening.toFixed(2)}`, bold:true  },
                { label:"Shift Start Overage / Shortage",count:"",          amount:`${cs.code} 0.00`,               bold:false },
                { label:"Cash Received",                 count:cs.inCount,  amount:`${cs.code} ${cs.totIn.toFixed(2)}`,bold:false },
                { label:"Cash Paid Out",                 count:cs.outCount, amount:`${cs.code} ${cs.totOut.toFixed(2)}`,bold:false},
                { label:"Shift End Overage / Shortage",  count:"",          amount:`${cs.code} ${cs.variance.toFixed(2)}`, bold:false },
                { label:"Drawer Balance",                count:"",          amount:`${cs.code} ${cs.counted.toFixed(2)}`, bold:true },
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const ccySummary = buildCloseDrawerAccounting({ shift: activeShift, drawer: selD, txns }).summary
    .map(row => {
      const c = CURRENCIES.find(x=>x.code===row.code)||{symbol:row.code,code:row.code};
      return { ...row, symbol:c.symbol, name:c.name, bal:row.expected };
    });
  const ccysInShift = ccySummary.map(row => row.code);
  ...
  const validCashMovement = () => {
    if (positiveFiniteAmount(amt) === null) return false;
    return ccy === "USD" || positiveFiniteAmount(useRate) !== null;
  };
  const doAdd = () => { if(!validCashMovement()) return; setTxns(p=>[buildTxn(1),...p]); setAmt(""); setNote(""); setCcy("USD"); setFxRate(""); setAddOpen(false); };
  const doRem = () => { if(!validCashMovement()) return; setTxns(p=>[buildTxn(-1),...p]); setAmt(""); setNote(""); setCcy("USD"); setFxRate(""); setRemOpen(false); };
...
function PettyCashPage({ drawers, shifts }) {
  ...
  /* ── helpers ── */
  const activeShiftId = fund => shiftIdForPettyFund(fund, shifts);

  const saveCreate = () => {
    const openingAmt = positiveFiniteAmount(ff.openingAmt);
    const replenishAt = positiveFiniteAmount(ff.replenishAt) ?? 100;
  ...
  const savePay = () => {
    ...
    const payment = canPayPettyCash(live, pf.amount);
    if (!payment.ok) {
      window.alert(payment.reason);
      return;
    }
  ...
  const saveRep = () => {
    if (!rf.amount) return;
    const amt = positiveFiniteAmount(rf.amount);
    if (amt === null) return;
    const txn = {

...
const summaryStartBal = cs => cs.openingBal ?? 0;
const summaryCashDrop = (shift, cs) => cs.cashDrop ?? (cs.code==="USD" ? (shift.cashDrop||0) : 0);
const summaryEndingBal = (shift, cs) => cs.endingBal ?? ((cs.bal||0) - summaryCashDrop(shift, cs));
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const activeShiftTxns = (txns||[]).filter(t => t.shiftId===activeShift?.id);
  const closeAccounting = buildCloseAccounting({
    shift: activeShift || {},
    drawer: selected || {},
  ...
  const usdCounted  = closeAccounting.byCode.USD?.countedBal || 0;
  const variance    = closeAccounting.variance;
  const mismatch    = variance !== 0;
  ...
  const handleClose = () => {
    ...
    const { closingBal, ccySummary, cashDrop: closeDrop, endingCcyBalances } = closeAccounting;
    ...
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt,
          balance:endingCcyBalances.USD || 0, ccyBalances:endingCcyBalances }
      : d));
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${summaryStartBal(cs).toFixed(2)}`, bold:true  },
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${summaryStartBal(cs).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${summaryStartBal(cs).toFixed(2)}`).join("\n") : "USD 0.00";
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  // Per-currency summary data includes opening balances plus signed movement.
  const ccySummary = buildShiftCurrencySummary({
    shift: activeShift || {},
    drawer: selD || {},
    txns: shiftTxns,
  }).map(row => {

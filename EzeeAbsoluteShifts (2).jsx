...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const sysBal      = closeAccounting.expectedBalances.USD || 0;
  const usdCounted  = closeAccounting.countedBalances.USD || 0;
  const variance    = closeAccounting.varianceByCode.USD || 0;
  ...
  const handleClose = () => {
    ...
    const usdSummary = closeAccounting.ccySummary.find(cs => cs.code==="USD");
    setShifts(p => p.map(s => s.id===selected.currentShift
      ? { ...s, closedAt,
          closingBal:closeAccounting.closingBal,
          status:"Closed",
          variance:closeAccounting.variance,
          ccySummary:closeAccounting.ccySummary,
          cashIn:usdSummary?.totIn||0,
          cashOut:usdSummary?.totOut||0,
          cashDrop:closeAccounting.cashDrop,
          closeNotes:notes,
    ...
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d,
          ...
          currentShift:null,
          lastClosed:closedAt,
          balance:closeAccounting.endingBal,
          ccyBalances:closeAccounting.endingBalances }
      : d));
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom:`1px solid ${T.rowBorder}`, verticalAlign:"top" }}
                      ...
                      <td style={{ padding:"10px 10px", textAlign:"right", color:T.txtMid }}>
                        {mlPrint(s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.cashDrop||0).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`)}
                      </td>
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  // Per-currency summary data includes the opening native balance plus live transactions.
  const ccySummary = buildCloseDrawerAccounting({
    shift: activeShift,
    drawer: selD,
    transactions: txns,
  }).ccySummary.map(row => {

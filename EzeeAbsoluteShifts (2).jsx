...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const shiftTxnsForActive = activeShift ? (txns||[]).filter(t => t.shiftId===activeShift.id) : [];
  const sysByCodeForActive = getSystemBalanceByCode(activeShift, selected, shiftTxnsForActive);
  const sysBal      = sysByCodeForActive[BASE_CCY] || 0;
  const usdCounted  = parseAmt(ccyCounts[BASE_CCY] || 0);
  ...
  const handleClose = () => {
    ...
    const closedSnapshot = buildClosedDrawerSnapshot({
      activeShift,
    ...
    setShifts(p => p.map(s => s.id===selected.currentShift
      ? { ...s, closedAt, closingBal:closedSnapshot.closingBal, status:"Closed",
          variance:closedSnapshot.variance, ccySummary:closedSnapshot.ccySummary,
          cashDrop:closedSnapshot.cashDrop, cashIn:closedSnapshot.cashIn,
          cashOut:closedSnapshot.cashOut, closeNotes:notes, openNotes:s.openNotes||"" }
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt,
          balance:closedSnapshot.drawerBalance, ccyBalances:closedSnapshot.ccyBalances }
      : d));
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${cashDropForCode(s.cashDrop, cs.code).toFixed(2)}`, bold:false },
                    { label:"Ending Balance",                 count:"",          amount:`${cs.code} ${(cs.bal-cashDropForCode(s.cashDrop, cs.code)).toFixed(2)}`, bold:true },
                  ].map((row,i) => (
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.bal-cashDropForCode(s.cashDrop, cs.code)).toFixed(2)}`).join("\n")
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom:`1px solid ${T.rowBorder}`, verticalAlign:"top" }}
                      ...
                      <td style={{ padding:"10px 10px", textAlign:"right", color:T.txtMid }}>
                        {mlPrint(s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${cashDropForCode(s.cashDrop, cs.code).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`)}
                      </td>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.bal-cashDropForCode(s.cashDrop, cs.code)).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} 0.00`).join("\n") : "USD 0.00";
                      const dropB   = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${cashDropForCode(s.cashDrop, cs.code).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`;
                      return (
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            ...
            const drawerCcys = getShiftCurrencyCodes(activeShift, selected, shiftTxns);
            const sysByCode  = getSystemBalanceByCode(activeShift, selected, shiftTxns);
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {drawerCcys.map(code => {
                  const drawerBal = ccyCounts[code] !== undefined ? parseAmt(ccyCounts[code]) : (sysByCode[code] || 0);
                  const cashDrop  = code===BASE_CCY ? parseAmt(drop) : 0;
        ...
        {step===2 && (<>
          ...
          {(() => {
            ...
            return drawerCcys.map((code, ci) => {
              ...
              const drawerBal = ccyCounts[code] !== undefined ? parseAmt(ccyCounts[code]) : (sysByCode[code] || 0);
              const cashDrop  = isUSD ? parseAmt(drop) : 0;
              const endingBal = drawerBal - cashDrop;
              const rows2 = [

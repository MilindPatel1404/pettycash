...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closeCalc   = activeShift ? computeCashDrawerClose({ activeShift, selectedDrawer:selected, txns, ccyCounts, cashDropInput:drop }) : null;
  const usdSummary  = closeCalc?.ccySummary?.find(row => row.code==="USD");
  const usdCounted  = parseOptionalFiniteAmount(ccyCounts["USD"]);
  const mismatch    = usdCounted !== null && !!usdSummary?.variance;
  const variance    = mismatch ? usdSummary.variance : 0;
  ...
  const handleClose = () => {
    if (!sel || !activeShift || !selected) return;
    const closedAt   = nowStr();
    const closeAccounting = computeCashDrawerClose({ activeShift, selectedDrawer:selected, txns, ccyCounts, cashDropInput:drop });
    setShifts(p => p.map(s => s.id===selected.currentShift
      ? { ...s, closedAt,
          closingBal:closeAccounting.closingBal,
          cashIn:closeAccounting.cashIn,
          cashOut:closeAccounting.cashOut,
          status:"Closed",
          variance:closeAccounting.variance,
          ccySummary:closeAccounting.ccySummary,
          cashDrop:closeAccounting.cashDrop,
          cashDropByCurrency:closeAccounting.cashDropByCurrency,
    ...
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d,
          ...
          currentShift:null,
          lastClosed:closedAt,
          balance:closeAccounting.endingBalance,
          ccyBalances:closeAccounting.endingCcyBalances }
      : d));
      ...
      <div style={{ padding:"20px 28px", overflowY:"auto" }}>
        ...
        <div style={{ background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:6, overflow:"hidden", marginBottom:14 }}>
          ...
          {(s.ccySummary||[{code:"USD",inCount:0,outCount:0,totIn:s.cashIn||0,totOut:s.cashOut||0,bal:s.closingBal||0}]).map((cs,ci,arr) => {
            const startBal = openingBalanceForSummary(cs);
            const cashDrop = cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0);
            const endingBal = endingBalanceForSummary(s, cs);
            return (
            <div key={cs.code}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${startBal.toFixed(2)}`, bold:true  },
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${endingBalanceForSummary(s, cs).toFixed(2)}`).join("\n")
                    : `USD ${(s.closingBal||0).toFixed(2)}`;
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${openingBalanceForSummary(cs).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${endingBalanceForSummary(s, cs).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${openingBalanceForSummary(cs).toFixed(2)}`).join("\n") : "USD 0.00";
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            const rows = closeCalc?.ccySummary || [];
            return rows.map(({ code, expectedBal }) => (
              <div key={code} style={{ marginBottom:14 }}>
                <R2>
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {rows.map(({ code, bal, cashDrop, endingBal }) => {
                  return (
                    <div key={code} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:12, alignItems:"center" }}>
                      <div style={{ padding:"8px 12px", border:`1px solid ${T.inputBdr}`, borderRadius:4, fontSize:13, color:T.txtMid, background:"#FAFAFA" }}>{code} {bal.toFixed(2)}</div>
                      {code==="USD"
        ...
        {step===2 && (<>
          ...
          {(() => {
            const rows = closeCalc?.ccySummary || [];
            return rows.map((summary, ci) => {
              const { code, openingBal, inCount, outCount, totIn, totOut, bal, cashDrop, endingBal } = summary;
              const rows2 = [
                ...
                { label:"Drawer Balance",                count:"",       amount:`${code} ${bal.toFixed(2)}`, bold:true },

...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closeAccounting = buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns:txns||[], counts:ccyCounts, cashDrop:drop });
  const closeCcyCodes = closeAccounting.ccySummary.map(row => row.code);
  const variance    = closeAccounting.variance;
  const mismatch    = Math.abs(variance) >= 0.01;
  ...
  const handleClose = () => {
    if (!sel || !selected?.currentShift || !activeShift) return;
    const closedAt   = nowStr();
    const accounting = buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns:txns||[], counts:ccyCounts, cashDrop:drop });
    setShifts(p => p.map(s => s.id===selected.currentShift
      ...
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt, balance:accounting.endingBal, ccyBalances:accounting.ccyBalances }
      : d));
      ...
      <div style={{ padding:"20px 28px", overflowY:"auto" }}>
        ...
        <div style={{ background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:6, overflow:"hidden", marginBottom:14 }}>
          ...
          {(s.ccySummary||[{code:"USD",inCount:0,outCount:0,totIn:s.cashIn||0,totOut:s.cashOut||0,bal:s.closingBal||0}]).map((cs,ci,arr) => {
            const countedBal = cs.countedBal ?? cs.bal ?? 0;
            const ccyDrop = cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0);
            const endingBal = cs.endingBal ?? (countedBal - ccyDrop);
            return (
            <div key={cs.code}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Drawer Balance",                 count:"",          amount:`${cs.code} ${countedBal.toFixed(2)}`, bold:true  },
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${ccyDrop.toFixed(2)}`, bold:false },
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.endingBal ?? ((cs.countedBal ?? cs.bal ?? 0) - (cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0)))).toFixed(2)}`).join("\n")
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.endingBal ?? ((cs.countedBal ?? cs.bal ?? 0) - (cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0)))).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
              ...
              <div key={code} style={{ marginBottom:14 }}>
                <R2>
                  ...
                  <Fld label="System Balance">
                    <div style={{ padding:"8px 12px", border:`1px solid ${T.inputBdr}`, borderRadius:4, fontSize:13, color:T.txtMid, background:"#FAFAFA" }}>
                      {code} {(closeAccounting.expectedBalances[code]||0).toFixed(2)}
                    </div>
                  </Fld>
                </R2>
              </div>
          ...
          {(() => {
            return (
              <>
                ...
                {closeAccounting.ccySummary.map(({ code, countedBal, cashDrop, endingBal }) => {
                  return (
                    <div key={code} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:12, alignItems:"center" }}>
                      <div style={{ padding:"8px 12px", border:`1px solid ${T.inputBdr}`, borderRadius:4, fontSize:13, color:T.txtMid, background:"#FAFAFA" }}>{code} {countedBal.toFixed(2)}</div>
                      {code==="USD"
        ...
        {step===2 && (<>
          ...
          {(() => {
            return closeAccounting.ccySummary.map((cs, ci) => {
              const { code, openingBal, inCount, outCount, totIn, totOut, countedBal, cashDrop, endingBal, variance:ccyVariance } = cs;
              const rows2 = [
                ...
                { label:"Shift End Overage / Shortage",  count:"",       amount:`${code} ${ccyVariance.toFixed(2)}`, bold:false },
                { label:"Drawer Balance",                count:"",       amount:`${code} ${countedBal.toFixed(2)}`, bold:true },
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const ccySummary = accessAccounting.ccySummary.map(row => {
    ...
    return { ...row, symbol:c.symbol, name:c.name, bal:row.expectedBal };
  });

...
const ccyStartBal = (cs) => Number(cs.openingBal ?? 0);
const ccyDropBal = (shift, cs) => cs.code===BASE_CCY ? Number(cs.cashDrop ?? shift.cashDrop ?? 0) : 0;
const ccyEndingBal = (shift, cs) => Number(cs.endingBal ?? ((cs.bal || 0) - ccyDropBal(shift, cs)));
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closePreview = activeShift ? buildCloseDrawerAccounting({ shift: activeShift, txns, countsByCode: ccyCounts, cashDrop: drop }) : null;
  const usdPreview  = closePreview?.ccySummary.find(cs => cs.code===BASE_CCY);
  const sysBal      = usdPreview?.expectedBal || 0;
  const usdCounted  = usdPreview?.countedBal || 0;
  const mismatch    = ccyCounts[BASE_CCY] !== undefined && closePreview?.variance !== 0;
  ...
  const handleClose = () => {
    ...
    const accounting  = buildCloseDrawerAccounting({ shift: activeShift, txns, countsByCode: ccyCounts, cashDrop: drop });
    const usdSummary  = accounting.ccySummary.find(cs => cs.code===BASE_CCY);
    setShifts(p => p.map(s => s.id===activeShift.id
      ? { ...s, closedAt, closingBal: accounting.closingBal, status:"Closed", variance: accounting.variance,
          cashIn: usdSummary?.totIn || 0, cashOut: usdSummary?.totOut || 0,
          ccySummary: accounting.ccySummary, cashDrop: accounting.cashDrop, closeNotes:notes, openNotes:s.openNotes||"" }
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt,
          balance: accounting.endingBalances[BASE_CCY] || 0, ccyBalances: accounting.endingBalances }
      : d));
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${ccyStartBal(cs).toFixed(2)}`, bold:true  },
                    ...
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${ccyDropBal(s, cs).toFixed(2)}`, bold:false },
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${ccyStartBal(cs).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom:`1px solid ${T.rowBorder}`, verticalAlign:"top" }}
                      ...
                      <td style={{ padding:"10px 10px", textAlign:"right", color:T.txtMid }}>
                        {mlPrint(s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${ccyDropBal(s, cs).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`)}
                      </td>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${ccyStartBal(cs).toFixed(2)}`).join("\n") : "USD 0.00";
                      const dropB   = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${ccyDropBal(s, cs).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`;
                      return (
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            return (closePreview?.ccySummary || []).map(cs => (
          ...
          {(() => {
            return (
              <>
                ...
                {(closePreview?.ccySummary || []).map(cs => {
                  return (
        ...
        {step===2 && (<>
          ...
          {(() => {
            const summaries = closePreview?.ccySummary || [];
            return summaries.map((cs, ci) => {
              const overShort = cs.countedBal - cs.expectedBal;
              const rows2 = [
                { label:"Starting Balance",              count:"",          amount:`${cs.code} ${cs.openingBal.toFixed(2)}`, bold:true  },
                { label:"Shift Start Overage / Shortage",count:"",          amount:`${cs.code} 0.00`,                 bold:false },
                { label:"Cash Received",                 count:cs.inCount,  amount:`${cs.code} ${cs.totIn.toFixed(2)}`,bold:false },
                { label:"Cash Paid Out",                 count:cs.outCount, amount:`${cs.code} ${cs.totOut.toFixed(2)}`,bold:false},
                { label:"Shift End Overage / Shortage",  count:"",          amount:`${cs.code} ${overShort.toFixed(2)}`, bold:false },
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const accessAccounting = activeShift ? buildCloseDrawerAccounting({ shift: activeShift, txns }) : null;
  ...
  const ccySummary = accessAccounting?.ccySummary || ccysInShift.map(code => {
                    ...
                    {ccySummary.map(s => (
                      <React.Fragment key={s.code}>
                        ...
                        <td style={{ padding:"9px 14px", textAlign:"right", fontWeight:row.bold?700:400, color:row.bold?T.txt:T.txtMid }}>
                          ...
                          :row.key==="start"   ? `${s.code} ${ccyStartBal(s).toFixed(2)}`
                  ...
                  <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:4, fontSize:12 }}>
                    ...
                    <tbody>
                      {[
                        ...
                        ["Starting Balance",               "", `${s.code} ${ccyStartBal(s).toFixed(2)}`, true,  false],

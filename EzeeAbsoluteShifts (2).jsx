...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closeShiftTxns = activeShift ? (txns||[]).filter(t => t.shiftId===activeShift.id) : [];
  const closeAccounting = activeShift
    ? buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns:closeShiftTxns, counts:ccyCounts, cashDrop:drop })
    : null;
  const variance    = closeAccounting?.byCode?.USD?.variance || 0;
  const mismatch    = Math.abs(variance) > 0.004;
  ...
  const handleClose = () => {
    ...
    const accounting = buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns:closeShiftTxns, counts:ccyCounts, cashDrop:drop });
    ...
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt,
          balance:accounting.endingBalance, ccyBalances:accounting.endingCcyBalances }
      : d));
      ...
      <div style={{ padding:"20px 28px", overflowY:"auto" }}>
        ...
        <div style={{ background:T.pageBg, border:`1px solid ${T.border}`, borderRadius:6, overflow:"hidden", marginBottom:14 }}>
          ...
          {(s.ccySummary||[{code:"USD",inCount:0,outCount:0,totIn:s.cashIn||0,totOut:s.cashOut||0,bal:s.closingBal||0}]).map((cs,ci,arr) => {
            const rowDrop = cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0);
            const endingBal = cs.endingBal ?? ((cs.bal||0) - rowDrop);
            const openingBal = cs.openingBal ?? 0;
            const varianceBal = cs.variance ?? 0;
            return (
            <div key={cs.code}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${openingBal.toFixed(2)}`,      bold:true  },
                    ...
                    { label:"Shift End Overage / Shortage",   count:"",          amount:`${cs.code} ${varianceBal.toFixed(2)}`,     bold:false },
                    { label:"Drawer Balance",                 count:"",          amount:`${cs.code} ${cs.bal.toFixed(2)}`,   bold:true  },
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${rowDrop.toFixed(2)}`,  bold:false },
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>{
                        const rowDrop = cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0);
                        return `${cs.code} ${(cs.endingBal ?? ((cs.bal||0)-rowDrop)).toFixed(2)}`;
                      }).join("\n")
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>{
                        const rowDrop = cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0);
                        return `${cs.code} ${(cs.endingBal ?? ((cs.bal||0)-rowDrop)).toFixed(2)}`;
                      }).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const accessAccounting = activeShift
    ? buildCloseDrawerAccounting({ drawer:selD, shift:activeShift, txns:shiftTxns })
    : null;
  const ccySummary = (accessAccounting?.ccySummary||[]).map(cs => {
    const c       = CURRENCIES.find(x=>x.code===cs.code)||{symbol:cs.code,code:cs.code};
                    ...
                    {ccySummary.map(s => (
                      <React.Fragment key={s.code}>
                        ...
                        <td style={{ padding:"9px 14px", textAlign:"right", fontWeight:row.bold?700:400, color:row.bold?T.txt:T.txtMid }}>
                          ...
                          :                      `${s.code} ${s.expectedBal.toFixed(2)}`}
                        </td>
                      </React.Fragment>
                    ))}
                  </tr>

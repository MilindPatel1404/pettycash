...
const summaryOpening = cs => parseFloat(cs?.opening ?? 0);
const summaryCashDrop = (cs, shift) => parseFloat(cs?.cashDrop ?? (cs?.code==="USD" ? (shift?.cashDrop||0) : 0));
const summaryEnding = (cs, shift) => parseFloat(cs?.ending ?? ((cs?.bal||0) - summaryCashDrop(cs, shift)));
const summaryVariance = cs => parseFloat(cs?.variance ?? 0);
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const handleClose = () => {
    ...
    const finalAccounting = buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns, ccyCounts, drop });
    const closedAt   = nowStr();
    setShifts(p => p.map(s => s.id===activeShift.id
      ? { ...s, closedAt, closingBal:finalAccounting.closingBal,
          cashIn:finalAccounting.cashIn, cashOut:finalAccounting.cashOut,
          status:"Closed", variance:finalAccounting.variance, ccySummary:finalAccounting.ccySummary,
          cashDrop:finalAccounting.cashDrop, closeNotes:notes, openNotes:s.openNotes||"" }
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt,
          balance:finalAccounting.endingBalances.USD || 0,
          ccyBalances:finalAccounting.endingBalances }
      : d));
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Shift End Overage / Shortage",   count:"",          amount:`${cs.code} ${summaryVariance(cs).toFixed(2)}`, bold:false },
    ...
    <>
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
          ...
          <>
            ...
            {(accounting?.ccySummary||[]).map(cs => (
              <div key={cs.code} style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:12, alignItems:"center" }}>
                ...
                <div style={{ padding:"8px 12px", border:`1px solid ${T.inputBdr}`, borderRadius:4, fontSize:13, color:T.txtMid, background:"#FAFAFA" }}>{cs.code} {cs.ending.toFixed(2)}</div>
              </div>
            ))}
        ...
        {step===2 && (<>
          ...
          {(() => {
            const rows = accounting?.ccySummary || [];
            return rows.map((cs, ci) => {
              const rows2 = [
                ...
                { label:"Ending Shift Balance",          count:"",          amount:`${cs.code} ${cs.ending.toFixed(2)}`, bold:true },
              ];
              return (
                <div key={cs.code}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  // Per-currency summary data includes the shift opening balance plus live cash movement.
  const ccySummary = activeShift && selD
    ? buildCloseDrawerAccounting({ drawer:selD, shift:activeShift, txns }).ccySummary.map(cs => {
        const c = CURRENCIES.find(x=>x.code===cs.code)||{symbol:cs.code,code:cs.code};
        return { ...cs, symbol:c.symbol, name:c.name };
      })
    : [];
                    ...
                    {ccySummary.map(s => (
                      <React.Fragment key={s.code}>
                        ...
                        <td style={{ padding:"9px 14px", textAlign:"right", fontWeight:row.bold?700:400, color:row.bold?T.txt:T.txtMid }}>
                          ...
                          :row.key==="start"   ? `${s.code} ${s.opening.toFixed(2)}`
                  ...
                  <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:4, fontSize:12 }}>
                    ...
                    <tbody>
                      {[
                        ...
                        ["Starting Balance",               "", `${s.code} ${s.opening.toFixed(2)}`, true,  false],

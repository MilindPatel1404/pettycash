...
const shiftOpeningByCode = (shift, code, fallbackCcyBalances={}) => {
  if (!shift) return 0;
...
const txnAmountByCode = (txn, code) => {
  if (code === BASE_CCY) return txn.fxCcy ? 0 : cashNum(txn.amount);
  return txn.fxCcy === code ? cashNum(txn.fxAmt) : 0;
};
...
const shiftExpectedBalances = (shift, txns=[], fallbackCcyBalances={}) =>
  Object.fromEntries(shiftCurrencyCodes(shift, txns, fallbackCcyBalances).map(code => [
    code,
    shiftOpeningByCode(shift, code, fallbackCcyBalances) +
      txns.reduce((sum, txn) => sum + txnAmountByCode(txn, code), 0)
  ]));

const cashDropForCode = (code, cashDrop=0) => code === BASE_CCY ? cashNum(cashDrop) : 0;
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const usdCounted  = cashNum(ccyCounts[BASE_CCY]);
  ...
  const handleClose = () => {
    ...
    const countedByCode = Object.fromEntries(drawerCcys.map(code => [
      code,
      ccyCounts[code] !== undefined ? cashNum(ccyCounts[code]) : (sysByCode[code] || 0)
    ]));
    ...
    const ccySummary = drawerCcys.map(code => {
      ...
      const getAmt = t => txnAmountByCode(t, code);
      ...
      return {
        ...
        startBal: shiftOpeningByCode(activeShift, code, selected?.ccyBalances),
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${cashNum(cs.startBal).toFixed(2)}`, bold:true  },
                    ...
                    { label:"Ending Balance",                 count:"",          amount:`${cs.code} ${(cs.bal-summaryCashDrop(s, cs)).toFixed(2)}`, bold:true },
                  ].map((row,i) => (
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${cashNum(cs.startBal).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${cashNum(cs.startBal).toFixed(2)}`).join("\n") : "USD 0.00";
              ...
              <div key={code} style={{ marginBottom:14 }}>
                <R2>
                  ...
                  <Fld label="System Balance">
                    <div style={{ padding:"8px 12px", border:`1px solid ${T.inputBdr}`, borderRadius:4, fontSize:13, color:T.txtMid, background:"#FAFAFA" }}>
                      {code} {cashNum(sysByCode[code]).toFixed(2)}
                    </div>
                  </Fld>
                </R2>
              </div>
          ...
          {(() => {
            return (
              <>
                ...
                {drawerCcys.map(code => {
                  const drawerBal = ccyCounts[code] !== undefined ? cashNum(ccyCounts[code]) : cashNum(sysByCode[code]);
                  const cashDrop  = cashDropForCode(code, drop);
        ...
        {step===2 && (<>
          ...
          {(() => {
            return drawerCcys.map((code, ci) => {
              ...
              const endingBal = drawerBal - cashDrop;
              const rows2 = [
                { label:"Starting Balance",              count:"",       amount:`${code} ${shiftOpeningByCode(activeShift, code, selected?.ccyBalances).toFixed(2)}`, bold:true  },
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const useRate   = ccy===BASE_CCY ? 1 : (cashNum(fxRate)||selCcyObj.defaultRate);
  const usdAmt    = ccy===BASE_CCY ? cashNum(amt) : toUSD(amt, useRate);
  ...
  // Determine all currencies present in this shift, including opening cash with no transactions yet.
  ...
  const ccySummary = ccysInShift.map(code => {
    ...
    const startBal = shiftOpeningByCode(activeShift, code, selD?.ccyBalances);
  ...
  const buildTxn = (sign) => {
    ...
    if (ccy !== BASE_CCY) { base.fxCcy=ccy; base.fxAmt=sign*cashNum(amt); base.fxRate=useRate; base.fxSymbol=selCcyObj.symbol; }
    return base;
  };
                  ...
                  <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:4, fontSize:12 }}>
                    ...
                    <tbody>
                      {[
                        ...
                        ["Starting Balance",               "", `${s.code} ${s.startBal.toFixed(2)}`, true,  false],

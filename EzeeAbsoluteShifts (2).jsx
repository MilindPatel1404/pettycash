...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const activeTxns  = activeShift ? (txns||[]).filter(t => t.shiftId===activeShift.id) : [];
  const activeAccounting = selected && activeShift
    ? buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns:activeTxns, countedBalances:ccyCounts, cashDrop:drop })
    : null;
  const sysBal      = activeAccounting?.expectedBalances?.USD || 0;
  const usdCounted  = activeAccounting?.countedBalances?.USD || 0;
  const mismatch    = !!activeAccounting?.enteredCounts?.USD && activeAccounting.variance !== 0;
  const variance    = activeAccounting?.variance || 0;
  ...
  const handleClose = () => {
    ...
    const accounting = buildCloseDrawerAccounting({
      drawer:selected,
      shift:activeShift,
      txns:(txns||[]).filter(t => t.shiftId===activeShift.id),
    ...
    setShifts(p => p.map(s => s.id===selected.currentShift
      ...
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt, balance:accounting.endingBalance, ccyBalances:accounting.ccyBalances }
      : d));
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.startingBal||0).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                  const dropBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${((cs.cashDrop ?? (cs.code==="USD" ? s.cashDrop : 0))||0).toFixed(2)}`).join("\n")
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.startingBal||0).toFixed(2)}`).join("\n") : "USD 0.00";
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            return (activeAccounting?.ccySummary||[]).map(cs => (
        ...
        {step===1 && (<>
          {(() => {
            const rows = activeAccounting?.ccySummary || [];
            return (
              <>
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  // Per-currency summary data includes the shift's opening native cash.
  const ccySummary = activeShift
    ? buildCloseDrawerAccounting({ drawer:selD, shift:activeShift, txns:shiftTxns }).ccySummary.map(row => {

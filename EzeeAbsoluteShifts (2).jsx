              ...
              <tr key={d.id} style={{ borderBottom:`1px solid ${T.rowBorder}` }}
                ...
                <td style={{ padding:"11px 14px" }}>
                  <div style={{ display:"flex", gap:8 }}>
                    ...
                    <button
                      onClick={() => { if (!d.inUseBy && !d.currentShift) setDrawers(p => p.filter(x => x.id!==d.id)); }}
                      disabled={!!d.inUseBy || !!d.currentShift}
                      title={d.inUseBy || d.currentShift ? "Close this drawer before deleting it." : "Delete drawer"}
                      style={{ background:"none", border:"none", cursor:(d.inUseBy||d.currentShift)?"not-allowed":"pointer", color:T.txtLight, opacity:(d.inUseBy||d.currentShift)?0.6:1, padding:2, display:"flex" }}>
                      {IC.trash}
                    </button>
                  </div>
                </td>
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const sysBal      = closeAccounting.systemByCode.USD || 0;
  ...
  const handleClose = () => {
    ...
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt,
          balance:accounting.endingBal, ccyBalances:accounting.endingCcyBalances }
      : d));
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${(cs.startingBal||0).toFixed(2)}`, bold:true  },
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${((cs.startingBal ?? 0)||0).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${((cs.startingBal ?? 0)||0).toFixed(2)}`).join("\n") : "USD 0.00";
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            const drawerCcys = closeAccounting.currencyCodes;
            const sysByCode  = closeAccounting.systemByCode;
        ...
        {step===1 && (<>
          {(() => {
            const drawerCcys = closeAccounting.currencyCodes;
            const countedByCode = closeAccounting.countedByCode;
            return (
              <>
                ...
                {drawerCcys.map(code => {
                  const summary = closeAccounting.ccySummary.find(cs => cs.code===code);
                  const drawerBal = countedByCode[code] || 0;
                  const endingBal = summary?.endingBal ?? drawerBal;
                  return (
        ...
        {step===2 && (<>
          ...
          {(() => {
            return closeAccounting.ccySummary.map((summary, ci) => {
              ...
              const drawerCcys = closeAccounting.currencyCodes;
              const rows2 = [
                { label:"Starting Balance",              count:"",       amount:`${code} ${summary.startingBal.toFixed(2)}`, bold:true  },
                ...
                { label:"Shift End Overage / Shortage",  count:"",       amount:`${code} ${summary.variance.toFixed(2)}`, bold:false },
                { label:"Drawer Balance",                count:"",       amount:`${code} ${summary.bal.toFixed(2)}`, bold:true },
                { label:"Cash Drop",                     count:"",       amount:`${code} ${summary.cashDrop.toFixed(2)}`,  bold:false },
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const useRate   = ccy==="USD" ? 1 : (fxRate==="" ? selCcyObj.defaultRate : parsedFxRate);
  const usdAmt    = ccy==="USD" ? parseFloat(amt||0) : toUSD(amt, useRate);
  const amountValid = positiveAmount(amt) !== null;
  const rateValid   = ccy==="USD" || (Number.isFinite(useRate) && useRate > 0);
  const cashMoveValid = amountValid && rateValid;
    ...
    <>
      ...
      {[{open:addOpen,onClose:()=>{setAddOpen(false);setCcy("USD");setFxRate("");},onSave:doAdd,title:"Add Cash",label:"Add Cash"},
        ...
        <DrawerPanel key={title} open={open} onClose={onClose} title={title} onSave={onSave} saveLabel={label} saveDisabled={!cashMoveValid}>
          ...
          {amt && !amountValid && <Alert type="warn">Amount must be greater than zero.</Alert>}
          {ccy !== "USD" && !rateValid && <Alert type="warn">Conversion rate must be greater than zero.</Alert>}
...
function PettyCashPage({ drawers, shifts }) {
  ...
  /* ── helpers ── */
  const activeShiftId = fund => resolvePettyCashShiftId({ fund, shifts });

  const saveCreate = () => {
    const openingAmt = positiveAmount(ff.openingAmt);
    if (!ff.name.trim() || openingAmt === null) return;
    setFunds(p => [...p, {
      ...ff, id:`pf${Date.now()}`,
      openingAmt,
  ...
  const savePay = () => {
    const validation = validatePettyCashPayout({ amount:pf.amount, currentBalance:live?.currentBalance });
    if (!pf.voucherRef || !validation.valid) return;
    const amt = validation.amount;
    const txn = {
  ...
  const saveRep = () => {
    const amt = positiveAmount(rf.amount);
    if (amt === null) return;
    const txn = {
  ...
  if (detail && live) {
    ...
    const payoutValidation = validatePettyCashPayout({ amount:pf.amount, currentBalance:live.currentBalance });
    const replenishAmount = positiveAmount(rf.amount);
    ...
    return (
      <>
        ...
        <DrawerPanel open={payOpen} onClose={()=>setPay(false)} title="Link Voucher Payout"
          onSave={savePay} saveLabel="Deduct from Fund" saveDisabled={!pf.voucherRef||!payoutValidation.valid}>
          <Alert type="info">
          ...
          {pf.amount && !payoutValidation.valid && <Alert type="warn">{payoutValidation.reason}</Alert>}
        ...
        <DrawerPanel open={repOpen} onClose={()=>setRep(false)} title="Replenish Fund"
          ...
          {rf.amount && replenishAmount===null && <Alert type="warn">Amount must be greater than zero.</Alert>}
  ...
  return (
    <>
      ...
      <DrawerPanel open={createOpen} onClose={()=>setCreate(false)} title="New Petty Cash Fund"
        onSave={saveCreate} saveLabel="Create Fund" saveDisabled={!ff.name||positiveAmount(ff.openingAmt)===null}>
        ...
        <R2>
          ...
          {ff.openingAmt && positiveAmount(ff.openingAmt)===null && <Alert type="warn">Opening amount must be greater than zero.</Alert>}

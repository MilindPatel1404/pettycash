...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closeAccounting = buildCashDrawerAccounting({
    drawer: selected,
    shift: activeShift,
    txns: shiftTxns,
    counts: ccyCounts,
  ...
  const mismatch    = ccyCounts["USD"] !== undefined && ccyCounts["USD"] !== "" && closeAccounting.variance !== 0;
  ...
  const handleClose = () => {
    ...
    const accounting = buildCashDrawerAccounting({
      drawer: selected,
      shift: activeShift,
      txns: shiftTxns,
      counts: ccyCounts,
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Ending Balance",                 count:"",          amount:`${cs.code} ${(cs.ending!==undefined?cs.ending:cs.bal-(cs.code==="USD"?(s.cashDrop||0):0)).toFixed(2)}`, bold:true },
                  ].map((row,i) => (
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.ending!==undefined?cs.ending:cs.bal-(cs.code==="USD"?(s.cashDrop||0):0)).toFixed(2)}`).join("\n")
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom:`1px solid ${T.rowBorder}`, verticalAlign:"top" }}
                      ...
                      <td style={{ padding:"10px 10px", textAlign:"right", color:T.txtMid }}>
                        {mlPrint(s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.code==="USD"?(s.cashDrop||0):0).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`)}
                      </td>
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.ending!==undefined?cs.ending:cs.bal-(cs.code==="USD"?(s.cashDrop||0):0)).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===0 && (<>
          ...
          {(() => {
            const drawerCcys = closeAccounting.ccySummary.map(cs=>cs.code);
            const sysByCode  = closeAccounting.expectedBalances;
          ...
          {(() => {
            ...
            return (
              <>
                ...
                {drawerCcys.map(code => {
                  const drawerBal = closeAccounting.countedBalances[code] ?? sysByCode[code] ?? 0;
                  const cashDrop  = code==="USD" ? closeAccounting.cashDrop : 0;
                  const endingBal = closeAccounting.endingCcyBalances[code] ?? (drawerBal - cashDrop);
                  return (
        ...
        {step===2 && (<>
          ...
          {(() => {
            return closeAccounting.ccySummary.map((cs, ci) => {
              ...
              const inCount   = cs.inCount;
              const outCount  = cs.outCount;
              const totIn     = cs.totIn;
              const totOut    = cs.totOut;
              const drawerBal = cs.bal;
              const cashDrop  = isUSD ? closeAccounting.cashDrop : 0;
              const endingBal = cs.ending;
              const rows2 = [
                { label:"Starting Balance",              count:"",       amount:`${code} ${(cs.opening||0).toFixed(2)}`, bold:true  },
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  // Per-currency summary data includes opening cash, not just shift deltas.
  const ccySummary = buildCashDrawerAccounting({
    drawer: selD,
    shift: activeShift,
    txns: shiftTxns,
  }).ccySummary.map(s => {
    const c       = CURRENCIES.find(x=>x.code===s.code)||{symbol:s.code,code:s.code,name:s.code};
                ...
                {ccySummary.map(s => (
                  <tr key={`tot-${s.code}`} style={{ background:T.thBg }}>
                    ...
                    <td style={{ padding:"8px 12px", textAlign:"right", fontWeight:700, color:T.txt, whiteSpace:"nowrap" }}>
                      {s.code} {(s.totIn-s.totOut).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>

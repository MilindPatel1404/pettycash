...
const summaryOpening = cs => parseFloat(cs.openingBal || 0);
const summaryDrop = (s, cs) => parseFloat(cs.cashDrop !== undefined ? cs.cashDrop : (cs.code==="USD" ? (s.cashDrop||0) : 0));
const summaryEnding = (s, cs) => parseFloat(cs.endingBal !== undefined ? cs.endingBal : ((cs.bal||0) - summaryDrop(s, cs)));
...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closeAccounting = activeShift
    ? buildCloseDrawerAccounting({ drawer:selected, shift:activeShift, txns:activeShiftTxns, countedBalances:ccyCounts, cashDrop:drop })
    : null;
  const sysBal      = closeAccounting?.summaryByCode?.USD?.expectedBal || 0;
  const usdCounted  = closeAccounting?.summaryByCode?.USD?.bal || 0;
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${summaryDrop(s, cs).toFixed(2)}`, bold:false },
                    { label:"Ending Balance",                 count:"",          amount:`${cs.code} ${summaryEnding(s, cs).toFixed(2)}`, bold:true },
                  ].map((row,i) => (
    ...
    <>
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${summaryEnding(s, cs).toFixed(2)}`).join("\n")
                  ...
                  const dropBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${summaryDrop(s, cs).toFixed(2)}`).join("\n")
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${summaryEnding(s, cs).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${summaryOpening(cs).toFixed(2)}`).join("\n") : "USD 0.00";
                      const dropB   = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${summaryDrop(s, cs).toFixed(2)}`).join("\n") : `USD ${(s.cashDrop||0).toFixed(2)}`;
                      return (
      ...
      <DrawerPanel open={panelOpen} onClose={() => setPanelOpen(false)} title="Close Cash Drawer" width={500} extraFooter={stepFooter}>
        ...
        {step===2 && (<>
          ...
          {(() => {
            const drawerCcys = closeAccounting?.ccySummary || [];
            return drawerCcys.map((cs, ci) => {
              const rows2 = [
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  // Per-currency summary data includes the shift's opening native balances.
  const accessAccounting = activeShift ? buildCloseDrawerAccounting({ drawer:selD, shift:activeShift, txns:shiftTxns }) : null;
  const ccySummary = (accessAccounting?.ccySummary || []).map(s => {
    const c = CURRENCIES.find(x=>x.code===s.code)||{symbol:s.code,code:s.code};
    return { ...s, symbol:c.symbol, name:c.name };
  });

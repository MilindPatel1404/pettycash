...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const closeAccounting = buildCloseDrawerAccounting({ shift: activeShift, drawer: selected, txns: (txns||[]).filter(t => t.shiftId===activeShift?.id), counts: ccyCounts, cashDrop: drop });
  ...
  const handleClose = () => {
    ...
    const accounting = buildCloseDrawerAccounting({ shift: activeShift, drawer: selected, txns: shiftTxns, counts: ccyCounts, cashDrop: drop });
    const usdSummary = accounting.ccySummary.find(cs => cs.code==="USD") || {};
    setShifts(p => p.map(s => s.id===selected.currentShift
      ? { ...s, closedAt, closingBal:accounting.closingBal, status:"Closed", variance:accounting.varianceByCode.USD||0,
          cashIn:usdSummary.totIn||0, cashOut:usdSummary.totOut||0, ccySummary:accounting.ccySummary,
          cashDrop:accounting.cashDrop, closeNotes:notes, openNotes:s.openNotes||"" }
      : s));
    setDrawers(p => p.map(d => d.id===selected.id
      ? { ...d, inUseBy:null, currentShift:null, lastClosed:closedAt, balance:accounting.endingBalance, ccyBalances:accounting.endingCcyBalances }
      : d));
          ...
          {(s.ccySummary||[{code:"USD",inCount:0,outCount:0,totIn:s.cashIn||0,totOut:s.cashOut||0,bal:s.closingBal||0}]).map((cs,ci,arr) => (
            <div key={cs.code}>
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {(() => {
                    const opening = cs.opening ?? 0;
                    const varianceAmt = cs.variance ?? 0;
                    const drawerBal = cs.counted ?? cs.bal ?? 0;
                    const cashDrop = cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0);
                    const endingBal = cs.endingBal ?? (drawerBal - cashDrop);
                    return [
                      { label:"Shift Start Overage / Shortage", count:"",          amount:`${cs.code} 0.00`,                         bold:false },
                      { label:"Starting Balance",               count:"",          amount:`${cs.code} ${opening.toFixed(2)}`,         bold:true  },
                      ...
                      { label:"Shift End Overage / Shortage",   count:"",          amount:`${cs.code} ${varianceAmt.toFixed(2)}`,     bold:false },
                      { label:"Drawer Balance",                 count:"",          amount:`${cs.code} ${drawerBal.toFixed(2)}`,       bold:true  },
                      { label:"Cash Drop",                      count:"",          amount:`${cs.code} ${cashDrop.toFixed(2)}`,        bold:false },
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const endingBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${(cs.endingBal ?? ((cs.counted ?? cs.bal) - (cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0)))).toFixed(2)}`).join("\n")
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const endB    = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${(cs.endingBal ?? ((cs.counted ?? cs.bal) - (cs.cashDrop ?? (cs.code==="USD" ? (s.cashDrop||0) : 0)))).toFixed(2)}`).join("\n") : `USD ${(s.closingBal||0).toFixed(2)}`;
...
function AccessPage({ drawers, shifts, txns: txnsProp, setTxns: setTxnsProp }) {
  ...
  const activeAccounting = buildCloseDrawerAccounting({ shift: activeShift, drawer: selD, txns: shiftTxns });
  const ccySummary = activeAccounting.ccySummary;
                    ...
                    {ccySummary.map(s => (
                      <React.Fragment key={s.code}>
                        ...
                        <td style={{ padding:"9px 14px", textAlign:"right", fontWeight:row.bold?700:400, color:row.bold?T.txt:T.txtMid }}>
                          ...
                          :row.key==="start"   ? `${s.code} ${(s.opening ?? 0).toFixed(2)}`
                  ...
                  <table style={{ width:"100%", borderCollapse:"collapse", marginBottom:4, fontSize:12 }}>
                    ...
                    <tbody>
                      {[
                        ...
                        ["Starting Balance",               "", `${s.code} ${(s.opening ?? 0).toFixed(2)}`, true,  false],

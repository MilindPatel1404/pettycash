...
function CloseDrawerPage({ drawers, setDrawers, shifts, setShifts, txns }) {
  ...
  const shiftTxnsForActive = (txns||[]).filter(t => t.shiftId===activeShift?.id);
  const drawerCcys  = activeShift ? shiftCurrencyCodes(activeShift, shiftTxnsForActive, selected?.ccyBalances) : [BASE_CCY];
  const sysByCode   = activeShift ? shiftExpectedBalances(activeShift, shiftTxnsForActive, selected?.ccyBalances) : { [BASE_CCY]:0 };
  ...
  const handleClose = () => {
    ...
    const {
      ...
      ccySummary,
      endingByCode,
    } = buildCloseAccounting({ activeShift, selectedDrawer:selected, txns:shiftTxns, ccyCounts, drop });
    setShifts(p => p.map(s => s.id===selected.currentShift
      ? { ...s, closedAt, closingBal, status:"Closed", variance:closeVariance, ccySummary,
              ...
              <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                ...
                <tbody>
                  {[
                    ...
                    { label:"Starting Balance",               count:"",          amount:`${cs.code} ${(cs.startBal||0).toFixed(2)}`, bold:true  },
              ...
              <tbody>
                {filteredShifts.length===0 ? (
                  ...
                  const startBal = s.ccySummary
                    ? s.ccySummary.map(cs=>`${cs.code} ${((cs.startBal||0)).toFixed(2)}`).join("\n")
                    : "USD 0.00";
                ...
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:11 }}>
                  ...
                  <tbody>
                    {filteredShifts.map(s => {
                      ...
                      const startB  = s.ccySummary ? s.ccySummary.map(cs=>`${cs.code} ${((cs.startBal||0)).toFixed(2)}`).join("\n") : "USD 0.00";
          ...
          {(() => {
            return (
              <>
                ...
                {drawerCcys.map(code => {
                  const drawerBal = cashNum(ccyCounts[code] || sysByCode[code]);
        ...
        {step===2 && (<>
          ...
          {(() => {
            ...
            return drawerCcys.map((code, ci) => {
              ...
              const getAmt   = t => txnAmountForCode(t, code);
